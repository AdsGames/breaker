...
particle_emitter::particle_emitter(vec2 position, vec2 size)
    : position(position), size(size), launch_velocity(vec2(-10, 10)), type(0) {
  // Load image
...
void particle_emitter::create_particle() {
  particle newPart(random(position.x, position.x + size.x),
                   random(position.y, position.y + size.y),
                   vec2(randomf(-0.1, 0.2), randomf(-0.4, -0.9)),
                   vec2(0.008, -0.008), vec2(random(5, 10)), 0xFFFFFF, 0x000000,
                   random(500, 1000), IMAGE, true);
