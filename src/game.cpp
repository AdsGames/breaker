...
game::game() {
  ...
  dialog_yes.SetImages("images/buttons/yes.png",
                       "images/buttons/yes_hover.png");
...
int game::select_block(int x, int y, int type = -1) {
  if (x < BLOCKS_WIDE && y < BLOCKS_HIGH && x >= 0 && y >= 0) {
    if ((type == -1 || MyBlocks[x][y].getType() == type) &&
      ...
      return 1 + select_block(x - 1, y, btype) + select_block(x + 1, y, btype) +
             select_block(x, y - 1, btype) + select_block(x, y + 1, btype);
    }
  }
...
void game::destroy_selected_blocks() {
  ...
  if (config_sound) {
    play_sample(block_break, 64 + num_destroyed * 5, 100,
                900 + num_destroyed * 80, 0);
  }
...
void game::update() {
  ...
  if (!game_over) {
    ...
    if (MouseListener::mouse_pressed & 1) {
      coordinate selected_block =
          get_block_index(MouseListener::x, MouseListener::y);

      if (selected_block.x != -1) {  // Ensure existing block
        if (blocks_selected > 1 &&
            block_at(selected_block.x, selected_block.y)->getSelected()) {
          destroy_selected_blocks();
          blocks_selected = 0;
        } else {
          deselect_blocks();
          blocks_selected = select_block(
  ...
  // Game over state
    ...
    // Set score
    if (score == 0) {
      score = difficulty * (((BLOCKS_WIDE * BLOCKS_HIGH) - count_blocks()) -
...
void game::draw() {
  ...
  // Draws text
  textprintf_right_ex(buffer, font, 1240, 16, makecol(0, 0, 0), -1,
                      "Blocks Left: %i", count_blocks());
  textprintf_ex(buffer, font, 40, 16, makecol(0, 0, 0), -1, "Time: %.0f",
                game_time.GetElapsedTime<seconds>());
  ...
  if (game_over) {
    ...
    textprintf_centre_ex(buffer, font, 640, 310, makecol(0, 0, 0), -1, "%s",
                         game_over_message.c_str());
    textprintf_centre_ex(buffer, font, 640, 360, makecol(0, 0, 0), -1,
                         "Score: %i", score);
